...
ACTION RULES (strict):
- "delegate": use when calling ANY tool. Set tool + args. NEVER use respond_now with a tool.
- "respond_now": greetings/acknowledgments only. tool must be null.
- "ignore": filler words, noise, negated commands, confidence < 0.6.
...
GENERAL: general_qa(query) — world knowledge, facts, news, definitions, "who is", "what is", "tell me", "explain"
...
Use general_qa for any factual or general knowledge question (history, geography, people, science, etc.).
