# ============================================================================
# DISABLED: PILOT-native services/ppt_template_builder.py implementation (pre-GD-template).
...
# ============================================================================
# DISABLED: services/ppt_template_builder.py transferred from
...
# ============================================================================
# ACTIVE: ppt_template_builder.py transferred from
# pilot-voice-agent-backend-ppt_update (newer snapshot, supersedes the
# feature/ppt-copilot @ 869dd08d9 version above). Import paths adapted to
# PILOT's backend.-prefixed convention. Adds a multi-provider LLM cascade
# (Gemini -> Groq -> Ollama, was Ollama-only) and speaker-notes generation
# (generate_slide_notes / generate_notes_for_deck) used by api/ppt.py's new
# _autofill_speaker_notes background task.
# ============================================================================
"""
Builds generated presentations by cloning slides out of the real Grid
Dynamics branded template instead of drawing shapes from scratch.
...
TEMPLATE_PATH = "data/ppt_templates/GD_template/GD Presentation template Updated.pptx"
TEMPLATE_PATH_MINIMALIST = "data/ppt_templates/GD_template/Latest - GD Presentation Template.pptx"
...
# The slide-number placeholder ('‹#›') shows up as its own text shape on
# almost every real content slide in this template — it must NOT count as
# "real content" on its own, or a slide with nothing else on it would still
# pass as non-blank.
_PAGE_NUMBER_PLACEHOLDER = "‹#›"


def _shapes_have_real_content(shapes) -> bool:
    for sh in shapes:
        if sh.shape_type == MSO_SHAPE_TYPE.PICTURE:
            return True
        if sh.has_text_frame:
            text = sh.text_frame.text.strip()
            if text and text != _PAGE_NUMBER_PLACEHOLDER:
                return True
    return False


def _slide_is_effectively_blank(slide) -> bool:
    """True only if NEITHER the slide's own shapes NOR its inherited layout
    have any real text or picture — a genuine "nothing here" slide. Some
    kinds (e.g. thank_you) legitimately have zero shapes of their own by
    design, getting their logo/text entirely from the slide's layout, so
    checking only slide.shapes would wrongly flag those as blank too."""
    if _shapes_have_real_content(slide.shapes):
        return False
    return not _shapes_have_real_content(slide.slide_layout.shapes)
...
# only one entry here had only one genuinely safe candidate; the rest were
# excluded for that reason, not overlooked.
# Re-verified 2026-07-15 against the ACTUAL current GD template (18 slides —
# the 2026-07-14 pass above was already stale by the time this was checked:
# it claimed a 39→25-slide shrink, but the live .pptx that ships today only
# has 18 slides, so indices 21/38/etc below were pointing past the end of
# the file — see README_PPT_COPILOT.md's note on the .odp→.pptx conversion).
# Every entry below was confirmed by actually running that kind's
# _LOCATORS[...] function against slide content dumped straight from the
# live file — not just "looks similar". team / speaker_1 / speaker_4 /
# key_message / table / chapter / thank_you have NO surviving candidate
# slide in either template (those layouts were removed in the redesign —
# "chapter"'s marker text, "This is a brief description", no longer appears
# anywhere in the deck; the closest thank_you candidate, GD slide 17, has an
# empty slide AND an empty layout — no "Thank you!" text survives anywhere
# in the current file) and are intentionally absent here;
# build_deck_from_template() skips any kind missing from this dict with a
# warning instead of crashing.
_KIND_SOURCES: dict[str, list[tuple[str, int]]] = {
    "cover":        [(GD,0),(GD,1),(GD,2),(GD,3),(GD,4),(GD,5),(MIN,0),(MIN,1)],
    "agenda":       [(GD,6)],
    "text":         [(GD,7),(GD,8),(GD,15)],
    "two_column":   [(GD,9)],
    "comparison":   [(GD,10)],
    "text_blocks":  [(GD,11)],
    "subsection":   [(GD,14)],
...
# actually built with, before per-slide source tracking existed. Used as the
# fallback when editing/extracting a slide whose sidecar has no recorded
# source (see api/ppt.py's kinds vs sources sidecars). Indices below for
# text_blocks/subsection/thank_you corrected 2026-07-15 to match the live
# 18-slide GD template (see _KIND_SOURCES above) — team/speaker_1/
# speaker_4/key_message/table/chapter have no surviving slide in the
# current file at all, so those are left pointing at their old (now
# out-of-range) positions; editing a pre-redesign deck's slide of one of
# those kinds with no recorded source is a pre-existing edge case, not
# something this fix covers.
_KIND_SOURCE_INDEX: dict[str, int] = {
    "cover": 0, "agenda": 6, "text": 7, "two_column": 9, "comparison": 10,
    "team": 11, "speaker_1": 12, "speaker_4": 13, "text_blocks": 11,
    "key_message": 19, "table": 23, "subsection": 14, "chapter": 29, "thank_you": 17,
}
...
def build_deck_from_template(content: dict, out_path: str, template_path: str = TEMPLATE_PATH) -> list[str | None]:
    ...
    for slide_data in content.get("slides", []):
        ...
        try:
            populate_slide_data(new_slide, kind, slide_data, source)
            if _slide_is_effectively_blank(new_slide):
                # No real text or picture anywhere on this slide (own shapes
                # or inherited layout) after population — never show this to
                # the user as a "blank"/logo-stripped slide in the deck.
                # _delete_slide_at targets its CURRENT last position: nothing
                # after it exists yet in this loop, so nothing else shifts.
                logger.warning(
                    f"Dropping {kind!r} slide (source {source_path}::{source_index}) — "
                    f"no real content after population"
                )
                _delete_slide_at(prs, len(prs.slides) - 1)
                sources_used.append(None)  # keep 1:1 alignment with content["slides"] / the kinds sidecar
            else:
                sources_used.append(f"{source_path}::{source_index}")
        except Exception as e:
            logger.error(f"Failed to populate {kind!r} slide: {e}", exc_info=True)
            # The clone was already added to prs above, before this try block
            # — left in place it would keep the source slide's own UNPOPULATED
            # default/demo content (e.g. "Lorem Ipsum...") in the final deck
            # rather than actually failing closed. Remove it instead.
            _delete_slide_at(prs, len(prs.slides) - 1)
            sources_used.append(None)
...
_NOTES_PROMPT = """\
Write EXACTLY 2 short, natural, conversational sentences of speaker notes
for someone about to present this slide out loud. The two sentences
TOGETHER must be no more than 25 words total (20-25 words) — be concise,
not a summary of everything on the slide. Base them ONLY on what this slide
...
Output the speaker notes as plain prose only — exactly 2 sentences, 20-25
words total, no markdown, no bullet points, no prefix like "Speaker notes:",
nothing else."""
...
def _ollama_call_notes(prompt: str):
    import ollama
    from backend.core.config import settings
    response = ollama.chat(
        model=settings.OLLAMA_MODEL,
        messages=[{"role": "user", "content": prompt}],
        think=False,
        options={"num_predict": 60},  # notes are capped at 2 sentences / 20-25 words
        stream=False,
    )
    return response.message.content if hasattr(response, "message") else response["message"]["content"]
...
async def generate_slide_notes(title: str, content: str) -> str | None:
    ...
    provider = "gemini"
    if not raw:
        raw = await _try_groq_text(prompt, max_tokens=60)  # notes are capped at 2 sentences / 20-25 words
        provider = "groq"
    if not raw:
        from backend.core.llm_gate import ollama_gate
        try:
