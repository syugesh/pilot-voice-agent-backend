# Error Code Reference: LOS_ALARM

**Document ID:** ERR-0003
**Category:** Error Code
**Applicable Device:** Gateway Router
**Applicable Technology:** GPON
**Severity:** Critical

## Summary
Reference entry describing the LOS_ALARM error, its trigger conditions, and the standard resolution workflow for Gateway Router units operating on GPON.

## Error Description
Loss of optical signal detected on ONT. This error typically surfaces in the device's system log and, on managed devices, is also reported to the ISP's TR-069/ACS management platform or SNMP trap receiver.
...
## Symptoms
- Gateway Router status LED reflects an error/alarm state (commonly solid red or fast-blinking amber).
...
3. Power cycle the Gateway Router and monitor for the error recurring within the first 10 minutes after reboot.
4. If the error persists, check physical layer statistics relevant to GPON (sync rate, optical power, RF levels) against normal operating thresholds.
...
## Escalation Rules
If LOS_ALARM recurs 3 or more times within a 24-hour period for the same device, escalate to Tier 2 Network Operations immediately, referencing this document ID. If the device is under warranty and hardware fault is confirmed, initiate an RMA per the Equipment Replacement SOP.

## Related Documents
- Troubleshooting Guide: TS-0004
- Product Manual: MAN-0004
- Firmware Notes: FW-0004

## Tags
error-code, los_alarm, gpon, gateway router, diagnostics
