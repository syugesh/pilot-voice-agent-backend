# Error Code Reference: DNS_UNREACHABLE

**Document ID:** ERR-0042
**Category:** Error Code
**Applicable Device:** VoIP ATA
**Applicable Technology:** Cable/DOCSIS
**Severity:** Critical

## Summary
Reference entry describing the DNS_UNREACHABLE error, its trigger conditions, and the standard resolution workflow for VoIP ATA units operating on Cable/DOCSIS.

## Error Description
Configured DNS servers not responding. This error typically surfaces in the device's system log and, on managed devices, is also reported to the ISP's TR-069/ACS management platform or SNMP trap receiver.
...
## Symptoms
- VoIP ATA status LED reflects an error/alarm state (commonly solid red or fast-blinking amber).
...
3. Power cycle the VoIP ATA and monitor for the error recurring within the first 10 minutes after reboot.
4. If the error persists, check physical layer statistics relevant to Cable/DOCSIS (sync rate, optical power, RF levels) against normal operating thresholds.
...
## Escalation Rules
If DNS_UNREACHABLE recurs 3 or more times within a 24-hour period for the same device, escalate to Tier 2 Network Operations immediately, referencing this document ID. If the device is under warranty and hardware fault is confirmed, initiate an RMA per the Equipment Replacement SOP.

## Related Documents
- Troubleshooting Guide: TS-0043
- Product Manual: MAN-0043
- Firmware Notes: FW-0043

## Tags
error-code, dns_unreachable, cable/docsis, voip ata, diagnostics
