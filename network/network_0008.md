...
## Summary
Technical reference document explaining vpn tunneling protocols supported on consumer routers for support staff who need to understand the underlying network architecture behind Verizon Fixed Wireless services when diagnosing complex customer issues.
...
## Technical Details
In a typical Verizon deployment, subscriber traffic originates at the customer's CPE (Fixed Wireless modem/router/ONT) and traverses the access network before reaching the aggregation layer. Key elements relevant to vpn tunneling protocols supported on consumer routers include:
...
## Tags
network-architecture, fixed wireless, verizon, tier2-reference, diagnostics
