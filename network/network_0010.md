...
## Summary
Technical reference document explaining cgnat (carrier-grade nat) impact on customers for support staff who need to understand the underlying network architecture behind Airtel GPON services when diagnosing complex customer issues.
...
## Technical Details
In a typical Airtel deployment, subscriber traffic originates at the customer's CPE (GPON modem/router/ONT) and traverses the access network before reaching the aggregation layer. Key elements relevant to cgnat (carrier-grade nat) impact on customers include:
...
## Tags
network-architecture, gpon, airtel, tier2-reference, diagnostics
