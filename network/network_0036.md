...
## Summary
Technical reference document explaining bng (broadband network gateway) role in subscriber authentication for support staff who need to understand the underlying network architecture behind Xfinity Fiber services when diagnosing complex customer issues.
...
## Technical Details
In a typical Xfinity deployment, subscriber traffic originates at the customer's CPE (Fiber modem/router/ONT) and traverses the access network before reaching the aggregation layer. Key elements relevant to bng (broadband network gateway) role in subscriber authentication include:
...
## Tags
network-architecture, fiber, xfinity, tier2-reference, diagnostics
