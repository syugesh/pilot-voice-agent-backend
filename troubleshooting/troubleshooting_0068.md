# Troubleshooting Guide: Port forwarding not working on Fixed Wireless Service

**Document ID:** TS-0068
**Category:** Troubleshooting
**Applies To:** AT&T Fixed Wireless, Gateway Router
**Severity:** Low
**Last Reviewed:** 2025-12-06

## Summary
Step-by-step diagnostic procedure for resolving port forwarding not working reported by subscribers on AT&T Fixed Wireless circuits terminating on a Gateway Router.

## Problem Description
Customers report port forwarding not working. This may manifest during peak hours (7 PM - 11 PM local time) or intermittently throughout the day, and is frequently accompanied by elevated packet loss or authentication retries.

## Symptoms
- Subscriber reports port forwarding not working, sometimes with duration and frequency data.
- Gateway Router status LEDs show abnormal state (blinking amber, solid red, or no PPP/online light).
...
## Root Cause
Investigation typically traces this issue to a corrupted PPPoE session on the BNG (Broadband Network Gateway) requiring session reset.

## Resolution Steps
1. Pull the Gateway Router event log via the local admin UI or remotely via TR-069/ACS and identify the timestamp of the last successful WAN authentication.
...
## Tags
troubleshooting, fixed wireless, gateway router, at&t, network-diagnostics
